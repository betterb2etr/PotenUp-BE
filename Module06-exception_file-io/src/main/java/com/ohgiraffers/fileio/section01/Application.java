 ...
 * 하지만 파일 작업은 외부 자원(하드디스크, 네트워크 등)에 의존하기에
 * 파일이 없거나 권한 부족 등 예외 상황이 자주 발생한다.
 *
 * 실생활 비유 : 파일 입출력은 마치 냉장고에서 읍식을 꺼내거나 넣는 것과 같다.
 * 냉장고 문을 열 떄 (파일 열기), 음식이 없으면(파일없음) 배고프거나 (예외발생)
 ...
 *
 * 메모리 구조 : 파일 입출력 시 데이터는 힙 영역에서 관리되며 file객체는 참조형으로 스택에 주소가 저장되고 실제 객체는 힙에 생성된다.
 * 입출력 스트림은 운영체제와 연결되며 성능적으로 버퍼를 사용하는 bufferReader/Writer 가 효율적이다.
 *
 */
