 ...
 * 예를 들어, 사용자가 0으로 나누기를 시도하거나, 파일을 열 떄 파일이 존재하지 않는 경우 등이 있다.
...
public class Application {
    public static void main(String[] args) {
        try {
            ...
            System.out.println("0으로 나누는게 정상이니.");
        } catch (Exception e) {

        } finally {
            System.out.println("execute without exception");
        }


    try{
        Application.checkAge(-2);
    } catch ( IllegalArgumentException e){
        e.printStackTrace();
        System.out.println("control Exception");
    }
    }
    ...
    public static void checkAge(int age){
        ...
        if(age<0){
            throw new IllegalArgumentException("나이는 0보다 어릴 수 없다");
        }
